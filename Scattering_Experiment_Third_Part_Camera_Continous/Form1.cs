...
namespace Scattering_Experiment
{
    ...
    public partial class acquireFrm : Form
    {
        ...
        private void btnCam1Start_Click(object sender, EventArgs e)
        {
            runCam1();
        }
        //Camera Control
        public void runCam1()
        {
            if (!bckWrkCam1.IsBusy)
                bckWrkCam1.RunWorkerAsync();
        }
        public void stopCam1()
        {
            if (bckWrkCam1.IsBusy)
            {
                bckWrkCam1.CancelAsync();
                MessageBox.Show(bckWrkCam1.CancellationPending.ToString());
            }
        }
        public void runCam2()
        {
            if (!bckWrkCam2.IsBusy)
                try
                {
                    bckWrkCam2.RunWorkerAsync();
                }
                catch (Exception ez)
                {
        ...
        public void stopCam2()
        {
            if (bckWrkCam2.IsBusy)
            {
                bckWrkCam2.CancelAsync();
                MessageBox.Show(bckWrkCam2.CancellationPending.ToString());
            }
        }

        private void bckWrkCam1_DoWork(object sender, DoWorkEventArgs e)
        {
            spinView cam = new spinView("19060459"); //"17197493" new spinView("19060459"); 22175210
            cam.initializeCamera();
            while (true)
            {
                
                cam.RunSingleCamera();
                img1 = cam.currentImage;
                //ThreadHelperClass_PicBox.SetImg(this, this.picBx1, img1);
                if (bckWrkCam1.CancellationPending)
                {
                    cam.closeAcquisition();
                    break;
                }
                //System.Threading.Thread.Sleep(1000);
                bckWrkCam1.ReportProgress(1);
            }
        }
        ...
        private void bckWrkCam2_DoWork(object sender, DoWorkEventArgs e)
        {
            spinView cam = new spinView("17197493"); //"17197493" new spinView("19060459");
            cam.initializeCamera();
            while (true)
            {
                cam.RunSingleCamera();
                Bitmap img = cam.currentImage;
                ThreadHelperClass_PicBox.SetImg(this, this.picBx2, img);
                if (bckWrkCam2.CancellationPending)
                {
                    cam.closeAcquisition();
                    break;
                }
        ...
        private void btnCam2Start_Click(object sender, EventArgs e)
        {
            runCam2();
        }
        ...
        private void btnCam1Start_Click_1(object sender, EventArgs e)
        {
            runCam1();
        }
        ...
        {
            string fileName = "E:/VB.NEt/Scattering_Experiment/Scattering_Experiment_Third_Part_Camera_Continous/Images/img1.bmp";

            snapshot1(fileName);
        }

        private void bckWrkCam1_ProgressChanged(object sender, ProgressChangedEventArgs e)
        {
            picBx1.Image = img1;
            picBx1.Refresh();
        }

        public void snapshot1(string fileName)
        {
            Bitmap img = img1;
            img.Save(fileName);
        }
