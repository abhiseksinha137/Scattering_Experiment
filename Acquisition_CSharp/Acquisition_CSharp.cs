...
namespace Acquisition_CSharp
{
    class Program
    {
        ...
        // cameras.
        int RunSingleCamera(IManagedCamera cam, string savePath)
        {
            int result = 0;
        ...
        static int Main(string[] args)
        {
            ...
            string saveName = "E:/VB.NEt/Scattering_Experiment/Acquisition_CSharp/Images/ImageFinal.jpg";
            ...
            foreach (IManagedCamera managedCamera in camList) using(managedCamera)
            {
                ...
                {
                    // Run example
                    result = result | program.RunSingleCamera(managedCamera,saveName);
                }
                catch (SpinnakerException )
                {
                    //Console.WriteLine("Error: {0}", ex.Message);
